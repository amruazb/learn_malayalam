...
const Day10 = () => {
  ...
  useEffect(() => {
    const checkUnlocked = async () => {
      if (user) {
        ...
        const day5Completed = progress.some(p => p.lesson_id === 'day-9' && p.completed)
  ...
  const handleTestComplete = async (result) => {
    ...
    if (user) {
      ...
      await saveProgress(user.id, 'day-10', result.score, completed)
    }
  }
  ...
  return (
    <div className="lesson-page">
      <header className="lesson-header">
        <h1>📅 Day {day10Data.day}: {day10Data.title}</h1>
        <p>{day10Data.description}</p>
      </header>
      ...
      {!showTest ? (
        <>
          ...
          {day10Data.sections.map((section, index) => (
          ...
          <div className="navigation-section">
            <Link to="/day/9" className="prev-lesson-btn">
              ← Day 5
            </Link>
        ...
        <MCQTest mcqs={day10Data.mcqs} onComplete={handleTestComplete} />
      ) : (
        <div className="result-card">
          ...
          {testResult.percentage >= 60 ? (
            <div className="result-message success">
              <p>🎉 Congratulations! You've completed Day 10!</p>
