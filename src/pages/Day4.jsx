...
const Day4 = () => {
  ...
  useEffect(() => {
    const checkUnlocked = async () => {
      if (user) {
        ...
        const day3Complete = progress.some(p => p.lesson_id === 'day-3' && p.completed)
        setIsUnlocked(day3Complete)
      } else {
        setIsUnlocked(true)
      }
      setLoading(false)
  ...
  const handleTestComplete = async (result) => {
    ...
    if (user) {
      ...
      await saveProgress(user.id, 'day-4', result.score, completed)
    }
  }
    ...
    return (
      <div className="lesson-page">
        <div className="locked-message">
          <h2>🔒 Day 4 is Locked</h2>
          <p>Complete Day 3 with at least 60% to unlock this lesson.</p>
          <Link to="/day/3" className="next-lesson-btn">Go to Day 3</Link>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="lesson-page">
      <header className="lesson-header">
        <h1>📅 Day {day4Data.day}: {day4Data.title}</h1>
        <p>{day4Data.description}</p>
      </header>
      ...
      {!showTest ? (
        <>
          ...
          {day4Data.sections.map((section, index) => (
          ...
          <div className="test-section">
            ...
            <p>Complete the quiz to unlock Day 5!</p>
            <button className="start-test-btn" onClick={() => setShowTest(true)}>Take Day 4 Test</button>
          </div>

          <div className="navigation-section">
            <Link to="/day/3" className="prev-lesson-btn">← Day 3</Link>
            <Link to="/days" className="next-lesson-btn">Back to Days</Link>
          </div>
        </>
      ) : !testResult ? (
        <MCQTest mcqs={day4Data.mcqs} onComplete={handleTestComplete} />
      ) : (
        <div className="result-card">
          ...
          {testResult.percentage >= 60 ? (
            <div className="result-message success">
              <p>🎉 Congratulations! You've completed Day 4!</p>
              <p>Day 5 is now unlocked!</p>
            </div>
          ) : (
          ...
          <div className="result-actions">
            ...
            {testResult.percentage >= 60 && <Link to="/day/5" className="next-lesson-btn">Continue to Day 5 →</Link>}
            <Link to="/days" className="next-lesson-btn">Back to Days</Link>
          </div>
        </div>
      )}
