...
const Day8 = () => {
  ...
  const handleTestComplete = async (result) => {
    ...
    if (user) {
      ...
      await saveProgress(user.id, 'day-8', result.score, completed)
    }
  }
  ...
  return (
    <div className="lesson-page">
      <header className="lesson-header">
        <h1>📅 Day {day8Data.day}: {day8Data.title}</h1>
        <p>{day8Data.description}</p>
      </header>
      ...
      {!showTest ? (
        <>
          ...
          {day8Data.sections.map((section, index) => (
        ...
        <MCQTest mcqs={day8Data.mcqs} onComplete={handleTestComplete} />
      ) : (
        <div className="result-card">
          ...
          {testResult.percentage >= 60 ? (
            <div className="result-message success">
              <p>🎉 Congratulations! You've completed Day 8!</p>
          ...
          <div className="result-actions">
            ...
            {testResult.percentage >= 60 && (
              <Link to="/day/9" className="next-lesson-btn">
                Continue to Day 7 →
              </Link>
            )}
