...
import formalData from '../data/formal-conversations.json'
import './LessonPage.css'

const FormalConversations = () => {
  const { user } = useAuth()
  ...
  const handleTestComplete = async (result) => {
    ...
    if (user) {
      ...
      await saveProgress(user.id, 'formal-conversations', result.score, completed)
    }
  }
  ...
  return (
    <div className="lesson-page">
      <header className="lesson-header">
        <h1>🤝 {formalData.title}</h1>
        <p>Professional and respectful communication in Malayalam</p>
      ...
      {!showTest ? (
        <>
          ...
          <div className="lessons-section">
            <h2>🤝 Formal Phrases</h2>
            <p className="section-description">Professional and respectful phrases for formal situations</p>
            <div className="lessons-grid">
              {formalData.lessons.map((item) => (
        ...
        <MCQTest mcqs={formalData.mcqs} onComplete={handleTestComplete} />
      ) : (
...
export default FormalConversations
